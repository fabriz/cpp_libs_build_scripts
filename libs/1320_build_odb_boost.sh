...
buildCurrentArchitecture__windows_msvc()
{
    ...
    # Patch files
    sed -i.orig -e '/<ImportLibrary>/d' -e 's/odb-d.lib/odb.lib/' ./odb/boost/libodb-boost-vc12.vcxproj
    ...
    prepareBuildStep "Building ${FM_CURRENT_ARCHITECTURE_LIB_TAG} ... "
    msbuild.exe libodb-boost-vc12.sln /toolsversion:${FM_TARGET_TOOLCHAIN_VERSION} /target:Build /property:Platform=${BUILD_PLATFORM}\
        /property:Configuration=${BUILD_CONFIGURATION} /property:TargetName="odb-boost" /property:IntermediateOutputPath="${LIBS_INTERMEDIATE_BUILD_WIN}"\
    ...
    copyFile ${FM_CURRENT_ARCHITECTURE_SOURCE_DIR}/tmp_build/odb-boost.dll ${FM_CURRENT_ARCHITECTURE_STAGE_DIR}/dll
    copyFile ${FM_CURRENT_ARCHITECTURE_SOURCE_DIR}/tmp_build/odb-boost.pdb ${FM_CURRENT_ARCHITECTURE_STAGE_DIR}/dll
    checkBuildStep
}

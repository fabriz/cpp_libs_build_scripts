...
decompressTarballForCurrentArchitecture()
{
    ...
    tar -xz -f ${LIB_TARBALL_LOCAL_PATH} -C ${FM_CURRENT_LIB_SOURCE_DIR} || error "Cannot decompress file ${LIB_TARBALL_LOCAL_PATH} to ${FM_CURRENT_LIB_SOURCE_DIR}"
    checkBuildStep

    mv "${FM_CURRENT_LIB_SOURCE_DIR}/icu" "${FM_CURRENT_ARCHITECTURE_SOURCE_DIR}"
}
...
buildCurrentArchitecture__android_clang()
{
    ...
    ./configure --host=${FM_TARGET_CROSS_COMPILER_HOST} ${BUILD_CONFIGURATION} --enable-shared=no --enable-static=yes\
        --with-cross-build=/fabri_libs/build/linux_gcc_x64_release/source/icu4c-62_1/x64/source\

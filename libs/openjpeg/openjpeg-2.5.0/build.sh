...
# Build script for openjpeg 2.5.0
...
{
    if [ -d "${FM_CURRENT_ARCHITECTURE_STAGE_DIR}"/lib/openjpeg* ]; then
        moveDirectory "${FM_CURRENT_ARCHITECTURE_STAGE_DIR}"/lib/openjpeg* "${FM_CURRENT_ARCHITECTURE_STAGE_DIR}"
    fi
}
...
buildCurrentArchitecture__linux_gcc()
{
    ...
    "${FM_CONFIG_CMAKE_COMMAND}" ${FM_TARGET_CMAKE_ARGUMENTS_GENERATE} \
        -DBUILD_SHARED_LIBS=False -DBUILD_CODEC=False > ${FM_CURRENT_ARCHITECTURE_LOG_FILE_CONFIGURE} 2>&1
    checkBuildStep

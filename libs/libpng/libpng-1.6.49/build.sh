...
# Build script for libpng 1.6.49
...
afterBuildCurrentArchitecture()
{
    ...
    if [ ${FM_TARGET_TOOLCHAIN} != "windows_msvc" ]; then
        ...
        copyFile "${FM_CURRENT_ARCHITECTURE_STAGE_DIR}/lib/libpng16_static.lib" "${FM_CURRENT_ARCHITECTURE_STAGE_DIR}/lib/libpng_static.lib"
    fi
...
buildCurrentArchitecture__windows_msvc()
{
    ...
    "${FM_CONFIG_CMAKE_COMMAND}" ${FM_TARGET_CMAKE_ARGUMENTS_GENERATE} \
        -DPNG_SHARED=False -DPNG_STATIC=True -DPNG_TESTS=False -DPNG_DEBUG_POSTFIX="" > ${FM_CURRENT_ARCHITECTURE_LOG_FILE_CONFIGURE} 2>&1
    checkBuildStep

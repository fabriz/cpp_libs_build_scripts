...
# Build script for botan2 2.19.2
...
buildCurrentArchitecture__linux_gcc()
{
    ...
    "${FM_CONFIG_PYTHON_COMMAND}" ./configure.py --os=linux --cc=gcc --cpu=${FM_TARGET_ARCHITECTURE} --ldflags="${LDFLAGS} -ldl"\
...
buildCurrentArchitecture__macos_clang()
{
    ...
    "${FM_CONFIG_PYTHON_COMMAND}" ./configure.py --os=macos --cc=clang --cpu=${FM_TARGET_ARCHITECTURE}\
        --cc-abi-flags="-arch ${FM_TARGET_ARCHITECTURE}" --disable-shared ${THIS_SCRIPT_OPTIONAL_BUILD_FLAGS} --without-documentation\
        --link-method=copy --prefix=${FM_CURRENT_ARCHITECTURE_STAGE_DIR} > ${FM_CURRENT_ARCHITECTURE_LOG_FILE_CONFIGURE} 2>&1
    checkBuildStep
...
buildCurrentArchitecture__ios_clang()
{
    ...
    prepareBuildStep "Configuring ${FM_CURRENT_ARCHITECTURE_LIB_TAG} ... "
    "${FM_CONFIG_PYTHON_COMMAND}" ./configure.py --os=ios --cpu=${LOCAL_CPU} \
        --cc-abi-flags="${ASMFLAGS}" --cxxflags="${CXXFLAGS}" --ldflags="${CXXFLAGS}" \
        --disable-shared ${THIS_SCRIPT_OPTIONAL_BUILD_FLAGS} --without-documentation \
        --link-method=copy --prefix=${FM_CURRENT_ARCHITECTURE_STAGE_DIR} > ${FM_CURRENT_ARCHITECTURE_LOG_FILE_CONFIGURE} 2>&1
    checkBuildStep

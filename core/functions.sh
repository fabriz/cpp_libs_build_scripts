...
{
    local LOCAL_CONFIGURE_DIR_NAME="tmp_build"
    local LOCAL_CONFIGURE_DIR="${FM_CURRENT_ARCHITECTURE_SOURCE_DIR}/${LOCAL_CONFIGURE_DIR_NAME}"
    
    createDirectory ${LOCAL_CONFIGURE_DIR_NAME}
    cd ${LOCAL_CONFIGURE_DIR_NAME}

    FM_TARGET_CMAKE_ARGUMENTS_GENERATE=${FM_TARGET_CMAKE_ARGUMENTS_GENERATE/-S ./-S ..}
}
...
initCurrentArchitecture()
{
    ...
    # CMake generate arguments configuration
    FM_TARGET_CMAKE_ARGUMENTS_GENERATE="-S . -B ."
    FM_TARGET_CMAKE_ARGUMENTS_GENERATE="${FM_TARGET_CMAKE_ARGUMENTS_GENERATE} -DCMAKE_PREFIX_PATH:PATH='${FM_LIBS_INSTALL_PREFIX}'"
    FM_TARGET_CMAKE_ARGUMENTS_GENERATE="${FM_TARGET_CMAKE_ARGUMENTS_GENERATE} -DCMAKE_INSTALL_PREFIX:PATH='${FM_CURRENT_ARCHITECTURE_STAGE_DIR}'"
    FM_TARGET_CMAKE_ARGUMENTS_GENERATE="${FM_TARGET_CMAKE_ARGUMENTS_GENERATE} -DCMAKE_MODULE_PATH:PATH='${FM_LIBS_INSTALL_CMAKE}'"

    # Corresponding C++ standard for CMake
    if [ ${FM_CONFIG_CXX_STANDARD} = "14" ]; then
        FM_TARGET_CMAKE_ARGUMENTS_GENERATE="${FM_TARGET_CMAKE_ARGUMENTS_GENERATE} -DCMAKE_CXX_STANDARD=14"
    elif [ ${FM_CONFIG_CXX_STANDARD} = "17" ]; then
        FM_TARGET_CMAKE_ARGUMENTS_GENERATE="${FM_TARGET_CMAKE_ARGUMENTS_GENERATE} -DCMAKE_CXX_STANDARD=17"
    elif [ ${FM_CONFIG_CXX_STANDARD} = "20" ]; then
        FM_TARGET_CMAKE_ARGUMENTS_GENERATE="${FM_TARGET_CMAKE_ARGUMENTS_GENERATE} -DCMAKE_CXX_STANDARD=20"
    else
    ...
    if [ ${FM_ARG_BUILD_VARIANT} = "debug" ]; then
        FM_TARGET_CMAKE_ARGUMENTS_GENERATE="${FM_TARGET_CMAKE_ARGUMENTS_GENERATE} -DCMAKE_BUILD_TYPE:STRING=Debug"
    elif [ ${FM_ARG_BUILD_VARIANT} = "release" ]; then
        FM_TARGET_CMAKE_ARGUMENTS_GENERATE="${FM_TARGET_CMAKE_ARGUMENTS_GENERATE} -DCMAKE_BUILD_TYPE:STRING=Release"
    elif [ ${FM_ARG_BUILD_VARIANT} = "profile" ]; then
        FM_TARGET_CMAKE_ARGUMENTS_GENERATE="${FM_TARGET_CMAKE_ARGUMENTS_GENERATE} -DCMAKE_BUILD_TYPE:STRING=RelWithDebInfo"
    else
    ...
    if [ -n "${FM_TARGET_CMAKE_TOOLCHAIN_FILE-}" ]; then
        FM_TARGET_CMAKE_ARGUMENTS_GENERATE="${FM_TARGET_CMAKE_ARGUMENTS_GENERATE} -DCMAKE_TOOLCHAIN_FILE:PATH='${FM_TARGET_CMAKE_TOOLCHAIN_FILE}'"
    fi

    # CMake build arguments configuration
    FM_TARGET_CMAKE_ARGUMENTS_BUILD="--build . -j ${FM_ARG_NUM_PROCESSES}"
    
    # CMake install arguments configuration
    FM_TARGET_CMAKE_ARGUMENTS_INSTALL="--install ."

    # Manage log verbosity
    if [ ${FM_CONFIG_VERBOSE_LOGS:-false} = true ]; then
        FM_TARGET_CMAKE_ARGUMENTS_GENERATE="${FM_TARGET_CMAKE_ARGUMENTS_GENERATE} -DCMAKE_FIND_DEBUG_MODE=True --log-level=TRACE"
        FM_TARGET_CMAKE_ARGUMENTS_BUILD="${FM_TARGET_CMAKE_ARGUMENTS_BUILD} --verbose"
        FM_TARGET_CMAKE_ARGUMENTS_INSTALL="${FM_TARGET_CMAKE_ARGUMENTS_INSTALL} --verbose"
    fi
}
